...
import { getActiveCamera2D, getWorld, type EntityRecord, type World } from "./world.js";
...
const tilemaps = new Map<number, { tileSize: number; cols: number; rows: number; tiles: number[]; tileset: number }>();
let nextTilemapId = 1;

export function tilemapLoadFromData(
  tileSize: number,
  cols: number,
  rows: number,
  tiles: number[],
...
export function tilemapGet(id: number) {
  return tilemaps.get(id | 0) ?? null;
}

export function tilemapAttach(entityId: number, tilemapId: number, world: World = getWorld()): void {
...
export function parseTilemapJson(text: string, defaultTileset = 0): number {
  ...
  return tilemapLoadFromData(
    data.tile_size ?? 32,
    data.cols ?? 0,
    data.rows ?? 0,
    data.tiles ?? [],
    data.tileset ?? defaultTileset
  );
}
...
export function renderWorld2d(options: Render2dOptions, world: World = getWorld()): void {
  ...
  for (const e of list) {
    if (e.tilemap) drawTilemap(ctx, e, options.getBitmap, canvas.width, canvas.height, world);
    if (e.sprite) drawSprite(ctx, e, options.getBitmap, canvas.width, canvas.height, world);
  }
}
