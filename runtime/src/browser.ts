...
export async function instantiateJuni(
  ...
  const engine = createEngineImports({
    ...
    cameraPerspective: (fov, aspect, near, far) =>
      scene3dHandlers.cameraPerspective?.(fov, aspect, near, far) ?? 0,
    cameraOrbit: (cam, tx, ty, tz, yaw, pitch, dist) =>
      scene3dHandlers.cameraOrbit?.(cam, tx, ty, tz, yaw, pitch, dist),
    ...
    scene3dSetAmbient,
    scene3dSetFog,
    initialScene: (options.initialScene as JScene | string | null) ?? null,
  });
