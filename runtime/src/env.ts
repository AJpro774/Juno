...
function stubF(): number {
  return 0;
}

const t0 = typeof performance !== "undefined" ? performance.now() : Date.now();

export function createPrintImports(memoryRef: MemoryRef, onPrint?: (text: string) => void) {
...
export function createEnvImports(options: EnvOptions = {}) {
  ...
  return {
    env: {
      ...
      webgpu_stub: (code: number) => stubs(code),
      ...
      canvas_fill_text:
        canvas.fillText ?? createCanvasFillTextFallback(memoryRef, options.onPrint),
      ...
      str_len: (ptr: number) => strLen(memoryRef, ptr),
      str_eq: (a: number, b: number) => strEq(memoryRef, a, b),
      clamp_f32: (x: number, lo: number, hi: number) => fr(Math.min(hi, Math.max(lo, x))),
      lerp_f32: (a: number, b: number, t: number) => fr(a + (b - a) * t),
      pow_f32: (x: number, y: number) => fr(Math.pow(x, y)),
      sign_f32: (x: number) => fr(x > 0 ? 1 : x < 0 ? -1 : 0),
      fmod_f32: (x: number, y: number) => fr(x % y),
      smoothstep_f32: (e0: number, e1: number, x: number) => {
        const t = fr(Math.min(1, Math.max(0, (x - e0) / (e1 - e0))));
        return fr(t * t * (3 - 2 * t));
      },
      deg_to_rad_f32: (d: number) => fr((d * Math.PI) / 180),
      rad_to_deg_f32: (r: number) => fr((r * 180) / Math.PI),
      ...
      pi_f32: () => fr(Math.PI),
      abs_i32: (x: number) => Math.abs(x | 0) | 0,
      min_i32: (a: number, b: number) => Math.min(a | 0, b | 0) | 0,
      max_i32: (a: number, b: number) => Math.max(a | 0, b | 0) | 0,
      clamp_i32: (x: number, lo: number, hi: number) => Math.min(hi | 0, Math.max(lo | 0, x | 0)) | 0,
      len2_f32: (x: number, y: number) => fr(Math.sqrt(x * x + y * y)),
      dot2_f32: (x1: number, y1: number, x2: number, y2: number) => fr(x1 * x2 + y1 * y2),
